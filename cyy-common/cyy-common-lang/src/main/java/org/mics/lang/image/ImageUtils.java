...
import org.mics.lang.exception.ImageException;
...
public class ImageUtils {
	 ...
	 * @return 抠出来的小图 
	 */
	public  static BufferedImage cutImage(FileInputStream fileInputStream,String imageType,int x,int y) {
		
        // 获取图片流
        Iterator<ImageReader> iterators= ImageIO.getImageReadersByFormatName(imageType);
		...
		try {
			...
			throw new ImageException("创建图片流出错！",e1);
		}
		...
		try {
			...
			throw new ImageException("图片读取到缓存区失败！",e);
		}
        return bi;
	}
	 ...
	 * @return
	 */
	public static String imageToBase64(BufferedImage image,String imageType){
        ByteArrayOutputStream byteArrayOutputStream=new ByteArrayOutputStream();
        try {
			ImageIO.write(image,imageType,byteArrayOutputStream);
		} catch (IOException e) {
			throw new ImageException("图片写入二进制流出错！",e);
		}
	...
	public static BufferedImage base64StringToImage(String base64String) {
        try {
        	...
        	throw new ImageException("base64转图片失败！",e);
        }
    }
     ...
     * @throws IOException
     */
    public static BufferedImage getRandomImage(String path,String suffix)  {
        File files = new File(path);
        File[] fileList = files.listFiles();
        List<String> fileNameList = new ArrayList<>();
        if (fileList!=null && fileList.length!=0){
            for (File tempFile:fileList){
                if (tempFile.isFile() && tempFile.getName().endsWith(suffix)){
        ...
        try {
			...
			throw new ImageException("读取文件失败",e);
		}
    }
}
