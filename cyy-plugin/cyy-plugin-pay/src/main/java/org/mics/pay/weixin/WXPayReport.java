...
public class WXPayReport {
    ...
    public static class ReportInfo {
        ...
        public String toLineString(String key) {
            ...
            try {
                String sign = SignUtil.HMACSHA256(sb.toString(), key);
                sb.append(sign);
                return sb.toString();
            }
            catch (Exception ex) {
    ...
    private WXPayReport(final WXPayProperties config) {
        this.config = config;
     ...
     * @return
     */
    public static WXPayReport getInstance(WXPayProperties config) {
        if (INSTANCE == null) {
